...
static int vlq_decode(int *dst,const uint8_t *src,int srcc) {
  if (srcc<1) return -1;
  if (!(src[0]&0x80)) {
    *dst=src[0];
    return 1;
    ...
    default: {
        ...
        switch (event->opcode) {
          ...
          case 0xf0: case 0xf7: { // Sysex, or Meta payload
              int paylen,seqlen;
              if ((seqlen=vlq_decode(&paylen,track->v+track->p,track->c-track->p))<1) return -1;
              track->p+=seqlen;
...
static int midi_track_acquire_delay(struct midi_file *file,struct midi_track *track) {
  int tickc,seqlen;
  if ((seqlen=vlq_decode(&tickc,track->v+track->p,track->c-track->p))<1) return -1;
  track->p+=seqlen;
  track->delay=tickc;
  return 0;
}
